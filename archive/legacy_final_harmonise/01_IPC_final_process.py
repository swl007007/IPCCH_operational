# Converted from Final_harmonise/01_IPC_final_process.ipynb
...
df = pl.read_csv('IPC_2017_2025_assembled.csv')
...
# %%
# read 1.Source Data\Outcome\IPC_2017_2025\ipc_outcome.csv
df_outcome = pl.read_csv('Outcome/IPC_2017_2025/ipc_outcome.csv')

# %%
# drop columns from df_outcome only if they exist
cols_to_drop = ['Unnamed: 0', 'lat', 'lon', 'row_id']
existing_cols = [col for col in cols_to_drop if col in df_outcome.columns]
df_outcome = df_outcome.drop(existing_cols)
...
df_final = df.join(df_outcome, on=['year', 'month', 'area_id'], how='left')
...
df_final.write_csv('IPC_2017_2025_final_v05022025.csv')
