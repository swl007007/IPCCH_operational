...
def process_tif_file(args):
    ...
    try:
        ...
        raise RuntimeError("Error processing {0}: {1}".format(filename, str(e)))
    finally:
        if checked_out:
...
if __name__ == "__main__":
    ...
    if not mean_results and not std_results:
        raise RuntimeError("No FLDAS extraction results were produced. Check raster inputs, Spatial Analyst license, and max_processes.")
    
    print("\nCreating final datasets...")
    
    # Define output files
    mean_output_csv = os.path.join(output_folder, "FLDAS_mean_extraction_results_p3.csv")
    std_output_csv = os.path.join(output_folder, "FLDAS_std_extraction_results_p3.csv")
