...
- After formal identity exists, write `error.json` and attempt the terminal `run_manifest.json`. If an ambiguous manifest write has a mismatched or unreadable exact-generation readback, never adopt or overwrite that unknown generation; return a structured formal-run `FAILED` result with `preflight: false`, `evidence_indeterminate: true`, `run_id`, `suite_version`, the original failure, and an explicit terminal-manifest evidence warning/error. Do not claim terminal-manifest success or route the run through preflight handling.
...
- Treat absent optional production state as generation zero at the explicit current-pointer, feature-month-pointer, and promotion-lease boundaries. Both local `FileNotFoundError` and production GCS `google.api_core.exceptions.NotFound` mean absence there; all other storage failures remain hard errors.
...
Before alias movement, call `get_ref` and generation-specific `read_bytes` to capture both `released/{feature_month}/production_suite_manifest.json` and `released/current.json` bytes/generations. At these optional captures and the promotion-lease capture, catch both local `FileNotFoundError` and production GCS `google.api_core.exceptions.NotFound` and represent absence as generation `0` with no bytes. After all aliases move:
...
- preflight deployment/source/discovery failure returns a structured error and CLI nonzero exit without formal run artifacts;
- every failure after successful discovery writes `error.json` and attempts terminal `run_manifest.json`; mismatched and unreadable ambiguous manifest readbacks remain formal-run `FAILED` outcomes with `preflight: false`, `evidence_indeterminate: true`, preserved `run_id`/`suite_version` and original failure, an explicit terminal-manifest evidence warning/error, no unknown-generation adoption or overwrite, and CLI nonzero exit;
- production-style `google.api_core.exceptions.NotFound` for an absent current pointer is treated as initial-release absence, and a complete first `run_latest` release succeeds without a pre-existing current pointer.
